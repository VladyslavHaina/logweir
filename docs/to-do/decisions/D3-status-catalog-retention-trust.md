...
- **Execution order per point** (so a half-deleted set never looks usable): write the
  create-only tombstone *intent*; delete the manifest; delete the segment objects; write the
  create-only tombstone *completion* with counts and failures. Evidence under `logweir/` is never
  touched, so the audit trail of a deleted point survives the point.
  **Amended 2026-09-17 at W9's fix round (review `d3w9` H3): the tombstones and the record are
  create-only and UNSIGNED.** Signing would make `logweir-retention` link `logweir-evidence`, whose
  reaching set `scripts/check-one-signer.sh` holds to `{logweir, e2e}`; widening that is a decision
  about the signer, taken in two files this worker does not own. Create-only under `logweir/`, with
  a credential that cannot delete there, is tamper-evident against the retention principal and
  against a delete-only principal — and not against one holding `s3:PutObject` under `logweir/`,
  which could pre-empt the key before the run writes it. `docs/stability.md` states both halves and
  forbids any surface from calling the record signed until a remedy lands.
