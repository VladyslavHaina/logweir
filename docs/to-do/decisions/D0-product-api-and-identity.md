...
**Amendment (2026-09-23, `claude/chart-poc`, reviewed and merged): the trusted proxy may be named by Service.** Where this record says "explicit trusted proxy CIDRs", read "explicit trusted proxy CIDRs **or** an explicitly named ingress-controller Service (`trustedProxyService: {namespace, name}`)". For a Service, the console trusts, as single hosts, the addresses of that Service's **serving** endpoints, read with one labelled `list` of `discovery.k8s.io/endpointslices` in that one namespace (the console's only grant there) and re-read every 5 s. The set fails closed: nothing is trusted before the first read and `/readyz` is false until it lands; a failed read never renews the set, which is distrusted 30 s after the last complete read; a list longer than one page is refused. The static list keeps its `/16` (IPv4) and `/48` (IPv6) width floors under `requireTrustedProxy`. As before, the trusted proxy is never an identity input. Who can widen the trusted set: anyone who can edit that Service, write an EndpointSlice in its namespace, or create or relabel a Pod matching its selector there; for an ingress controller on `hostNetwork`, the node's address is trusted, and so is every host process on that node.
...
**Amendment (2026-09-23, PLAT-19.2 as landed on `claude/plat19-2`, review `claude/plat19-2.review.md` ACCEPT).** The signed policy reference is `policy {name, digest}`, not `{apiVersion, kind, name, uid, specDigest, bindingDigest}`, because the approval policy lives in the installation configuration (an immutable, content-addressed ConfigMap mounted into both the controller and the console) and not in a namespaced CRD: a namespace operator able to create such an object could bind their own namespace, which this record forbids ("not a namespace operator edit"). `digest` is the sha256 of the canonical policy snapshot and so covers what `specDigest` covered (name, mode, `maxAgeSeconds`, `requireDistinctPrincipal`); the binding is proved by the signed `subject.namespace`, compared with the namespace's current binding at every checkpoint (Approval controller, Restore admission, mounted bundle, runner), which is what `bindingDigest` did; `allowOrdinaryConfirmation` needs no signature because an Ordinary policy cannot exist without that floor; `apiVersion`/`kind`/`uid` have no object to name. Consequence, accepted: rebinding a namespace A → B → A re-accepts a not-yet-expired document issued under the unchanged policy A — the same policy, and this record requires re-confirmation only for a policy that differs. Also as landed: `ticket` is required at every checkpoint in Governed; `localAdmin` refuses Ordinary with `409 policy_mismatch` before anything is created (the console mode that "does not expose Ordinary"). Follow-up (defence in depth, not required): the controller could also refuse an Ordinary v2 document whose `requester.issuer` is `urn:logweir:local-admin`, so a mixed-version console rollout cannot get one admitted.
