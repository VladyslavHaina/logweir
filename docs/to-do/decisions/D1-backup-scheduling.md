...
4. `409` → re-read next reconcile. `422` (a legacy object that fails a newer schema) or `403` → record in `HistoryRetained=False reason=MigrationBlocked` naming up to 10 Backups; never retried faster than the inventory interval.
