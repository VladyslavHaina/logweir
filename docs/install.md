...
**Minimum Kubernetes: 1.29.** The nine CRDs use CEL validation rules, which are
...
back. Apply the new additive schemas, wait for all nine Logweir definitions to
...
for crd in approvals backupdestinations backups backupschedules kafkaclusters \
           preflights restores topicdiscoveries trustrosters; do
...
additive**: three new kinds — `BackupDestination`, `TopicDiscovery` and
`Preflight` — plus optional fields on `Backup`, `BackupSchedule` and `Restore`.
No existing object is converted, nothing is rewritten, and an object that names
none of the new fields resolves exactly as it did. An older controller reading a
...
Namespace, the nine CustomResourceDefinitions, the RBAC, the controller
...
the Namespace and everything inside it, the nine CRDs and all their custom
