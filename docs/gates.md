...
|---|---|---|---|
| `just e2e` | the whole product against a real broker, a real MinIO and a real archive: backup, restore, verify, signed evidence, both readers | the compose stack **up** (`just e2e-up`), and `.engine/kafka-backup` extracted | none directly — the suite is Rust, under `--features e2e` |
