...
| 23 | `just chart-check` | `scripts/check-chart.sh` (Task 35): `charts/logweir/crds/` is byte-identical to `config/crd/` (`cmp`) and `charts/logweir/ui/` does not exist — Task 39 moved the page into the `logweir-ui` image and `scripts/check-image-ui.sh` (`just smoke-ui`, in the table below) hashes what the image serves against `ui/`; `helm lint` passes for the defaults and every example; `helm template` regenerated into `charts/logweir/rendered/` leaves it byte-identical (the `crds-check` drift idiom); every rendered image is a digest EXCEPT the three Logweir images, which must be exactly `<repository>:latest` (the owner's decision of 2026-09-12 — a digest or any other tag there is a red, and `:latest` on any OTHER image is still refused), and except the author-only render, exempt by name — its premise is a locally built tag; the values schema refuses `--set demoKafka.enabled=yes`; `values.yaml` names the tree's own REPOSITORIES (derived from `config/manager/deployment.yaml` and `weirkeeper::job::RUNNER_IMAGE`, never spelt in the script — and `ui.image` as `logweir-ui` under the namespace those two share) at `:latest`. Refuses without helm >= 4, naming it | that the chart installs — that is `just helm-demo` and `helm-demo.yml`, in the table below | 3.8 |
...
**`just gate` deliberately runs none of these nine.** Each needs the compose
...
chart-check` line); the **three** below are here; seventeen exist.
...
| `just smoke-ui` | the UI image serves the tree: the fourteen shipped files are at `/ui` with their **sha256 equal to `ui/`'s** and nothing else is under `/ui` (no README, no `ui/tests/`, no key material), Logweir's `LICENSE`/`NOTICE` and kubectl's inventory and licence are present and byte-identical, no MIT notice and no Rust inventory are, `Dockerfile.ui`'s `FROM` and the image's own base label and baked inventory name one digest, the entrypoint is `kubectl` with no `CMD`, and `kubectl version --client` runs. **This is what replaced `check-chart.sh`'s byte-copy arm** (Task 39): the old arm compared two directories in this repository, this one compares the bytes a browser receives. `--no-exec` adds an AArch64 assertion and runs nothing, for a variant the host cannot execute | a Docker daemon, and `just image-ui` (which it depends on) | `scripts/check-image-ui.sh` |
