...
# job is deferred to Task 13/21c): A SUCCESSFUL SEED DIRTIES THE TREE BY DEFAULT.
# (Set LOGWEIR_SEED_REFRESH_FIXTURES=0 and it does not touch a tracked file at
# all — that is what `scripts/demo.sh` does, so the quickstart leaves a clean
# tree. Everything below describes the DEFAULT, refreshing path.)
...
# --------------------------------------------------------------------------
# REFRESHING THE TRACKED FIXTURES IS OPT-OUT (Task 22 fix round 1, FIX 1).
# --------------------------------------------------------------------------
# This script does two separable jobs: it SEEDS the stack (produce records,
# take a backup) and it REFRESHES two CHECKED-IN fixtures from the resulting
# archive. Only the first is what a drill needs.
#
# `scripts/demo.sh` is the quickstart a stranger runs, and it needs the seed
# and nothing else — so it sets LOGWEIR_SEED_REFRESH_FIXTURES=0 and the two
# `mv`s at the bottom are skipped. Before this existed, running the quickstart
# left `git status` showing two modified tracked files with no explanation,
...
# rather than trusting that skipping two `mv`s was enough. A guard that only
# exists in the mode nobody runs is not a guard.
before_manifest=$(shasum -a 256 "$FIXTURE_MANIFEST" | cut -d" " -f1)
before_segment=$(shasum -a 256 "$FIXTURE_SEGMENT" | cut -d" " -f1)
...
if [ "$REFRESH_FIXTURES" = "0" ]; then
  ...
  # run leaves the working tree exactly as it found it. If a future edit adds
  # another writer to either path, this fails here instead of surfacing as an
  # unexplained `git status` on an adopter's machine.
  after_manifest=$(shasum -a 256 "$FIXTURE_MANIFEST" | cut -d" " -f1)
  after_segment=$(shasum -a 256 "$FIXTURE_SEGMENT" | cut -d" " -f1)
  [ "$before_manifest" = "$after_manifest" ] \
    || die "$FIXTURE_MANIFEST was modified with LOGWEIR_SEED_REFRESH_FIXTURES=0"
  [ "$before_segment" = "$after_segment" ] \
    || die "$FIXTURE_SEGMENT was modified with LOGWEIR_SEED_REFRESH_FIXTURES=0"
  echo "    verified: both tracked fixtures are byte-identical to before this ran."
else
  mv "$stage/manifest.json" "$FIXTURE_MANIFEST"
  mv "$stage/segment.kbak"  "$FIXTURE_SEGMENT"

  echo "==> seeded. manifest -> $FIXTURE_MANIFEST"
  echo "               segment -> $FIXTURE_SEGMENT (real 0.21.0 bytes)"
  ...
  echo "    Set LOGWEIR_SEED_REFRESH_FIXTURES=0 to seed WITHOUT touching them"
  echo "    (which is what scripts/demo.sh does)."
fi
