...
# ---------------------------------------------------------------------------
#   1. THE SERVED FILES. `/ui` inside the image holds exactly the sixteen
...
# =========================================================================
echo "-- check 1 (the page): /ui is the sixteen shipped files, sha256 for sha256, and nothing else"
if ! cp_out /ui "$work/ui"; then
  fail "check 1 (the page): $ref carries no /ui directory." \
       "  \`Dockerfile.ui\` COPYs the sixteen shipped files there and the chart's" \
...
if [ "$tree_count" -ne 16 ]; then
  fail "check 1 (the page): the tree holds $tree_count shipped UI file(s), not sixteen." \
...
if [ "$image_count" -ne 16 ]; then
  fail "check 1 (the page): $ref's /ui holds $image_count file(s), not sixteen." \
       "  It must hold the sixteen shipped files and NOTHING else: no README.md," \
...
echo "   ok: sixteen files, sha256 for sha256, and nothing else under /ui"
...
for f in LICENSE NOTICE; do
  if ! cp_out "/usr/share/licenses/logweir/$f" "$work/logweir-$f"; then
    fail "check 2 (licence): $ref is missing /usr/share/licenses/logweir/$f." \
       "  The sixteen files it serves are Logweir's own Apache-2.0 code, and" \
...
if cp_out /usr/share/licenses/logweir/THIRD_PARTY_NOTICES.md "$work/tpn"; then
  fail "check 2 (licence): $ref CARRIES /usr/share/licenses/logweir/THIRD_PARTY_NOTICES.md." \
       ...
       "  crate — it is sixteen static files over a kubectl — so shipping it would" \
...
if [ "$no_exec" -eq 1 ]; then
  echo
  echo "ok: --no-exec — an AArch64 image, the sixteen shipped files byte for byte, the licences"
...
echo
echo "ok: the sixteen shipped files byte for byte at /ui and nothing else, Logweir's licences,"
