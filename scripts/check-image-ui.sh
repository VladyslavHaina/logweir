...
# ---------------------------------------------------------------------------
#   1. THE SERVED FILES. `/ui` inside the image holds exactly the twenty-two
...
if ! cp_out /ui "$work/ui"; then
  fail "check 1 (the page): $ref carries no /ui directory." \
       "  \`Dockerfile.ui\` COPYs the twenty-two shipped files there and the chart's" \
...
for f in LICENSE NOTICE; do
  if ! cp_out "/usr/share/licenses/logweir/$f" "$work/logweir-$f"; then
    fail "check 2 (licence): $ref is missing /usr/share/licenses/logweir/$f." \
       "  The twenty-two files it serves are Logweir's own Apache-2.0 code, and" \
...
if cp_out /usr/share/licenses/logweir/THIRD_PARTY_NOTICES.md "$work/tpn"; then
  fail "check 2 (licence): $ref CARRIES /usr/share/licenses/logweir/THIRD_PARTY_NOTICES.md." \
       ...
       "  crate — it is twenty-two static files over a kubectl — so shipping it would" \
...
if [ "$no_exec" -eq 1 ]; then
  echo
  echo "ok: --no-exec — an AArch64 image, the twenty-two shipped files byte for byte, the licences"
...
echo
echo "ok: the twenty-two shipped files byte for byte at /ui and nothing else, Logweir's licences,"
