...
main `fdb48cd8`, published by CI run 36183088296 (2026-09-25). It carries the
...
human-pass round (first published at `a54fb823`), P15 — a followed
readiness check is read until the longest time a check may take — with the
console's third human-pass round (first published at `815249cb`), and P16 — a
repaint keeps the reader's place, and a step opens at its heading — with the
Catalog's code spans (O2). The PoC installed at `86a554e6` was upgraded in
place to `02dc44b6`, then to `b748fd5f`, to `a54fb823`, to `815249cb` and
then to `fdb48cd8` with
