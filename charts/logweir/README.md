...
| `Deployment`, `Service`, `ServiceAccount`, `ClusterRole`s, `RoleBinding` `<release>-ui` | `ui.enabled` | `kubectl proxy` serving the twenty UI files and the API on one origin, with its own authority (below). The files come from the image `ui.image`, not from a ConfigMap |
...
2026-09-12 — the command is below) with the **twenty shipped UI files copied
...
* `scripts/check-image-ui.sh` (`just smoke-ui`; needs a Docker daemon, so it is
  ...
  twenty files the `logweir-ui` image serves, sha256 for sha256 against
