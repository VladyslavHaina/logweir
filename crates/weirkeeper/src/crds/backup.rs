...
pub struct BackupEvidence {
    ...
    /// The immutable sha256 capture claim the runner reported for the exact
    /// receipt bytes it signed and uploaded at `receiptKey`, as
    /// `sha256:<lowercase hex>` — the one digest spelling this corpus uses
    /// everywhere (`logweir_core::ids::sha256_prefixed`). The controller
    /// never manufactures this status value from bytes it fetched.
    ///
    /// When the controller can read the destination, it independently hashes
    /// the fetched receipt and compares that observation with this claim;
    /// disagreement is `Invalid`. An older runner that reports only the two
    /// object keys supplies no immutable digest anchor: fetched evidence is
    /// `NotAttempted`/`legacy-unbound`, no controller-computed digest is
    /// published here, and no receipt facts are projected. Absence therefore
    /// remains backward-compatible without turning replacement bytes into a
